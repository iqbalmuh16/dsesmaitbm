...
function gen_menu()
{
	...
	if ($sess_level == "guru") {
		...
		$menu = array(
			...
			array("icon" => "th-large", "url" => "m_jurusan", "text" => "DATA JURUSAN"),
			array("icon" => "book", "url" => "m_mapel", "text" => "DATA MAPEL"),
			array("icon" => "edit", "url" => "m_soal", "text" => "DATA SOAL"),
			array("icon" => "file", "url" => "h_ujian", "text" => "HASIL UJIAN"),
	...
	if ($menu != null) {
		...
		foreach ($menu as $m) {
			if ($url == $m['url']) {
				...
				echo '<a href="' . base_url() . 'adm/' . $m['url'] . '" class="btn btn-sq " style="background-color: #2700C2; color:white"><i class="glyphicon glyphicon-' . $m['icon'] . ' g3x"></i><br><br/>' . $m['text'] . ' </a>';
			}
		}
