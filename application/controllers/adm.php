...
class Adm extends CI_Controller
{
	...
	public function h_ujian()
	{
		...
		if ($uri3 == "det") {
			...
			$q_datanya = $this->db->query("
	        	SELECT a.id, b.nama, b.jurusan, b.id_jurusan, a.nilai, a.jml_benar
				FROM tr_ikut_ujian a
				INNER JOIN m_siswa b ON a.id_user = b.id
				WHERE a.id_tes = '$uri4' 
			...
			foreach ($q_datanya as $d) {
				...
				$data_ok[1] = $d['nama'];
				$data_ok[2] = $d['jurusan'];
				$data_ok[3] = $d['id_jurusan'];
				$data_ok[4] = $d['jml_benar'];
				$data_ok[5] = $d['nilai'];
	...
	public function hasil_ujian_cetak()
	{
		...
		$a['hasil'] = $this->db->query("SELECT m_siswa.nama, tr_ikut_ujian.nilai, tr_ikut_ujian.jml_benar, m_siswa.jurusan, m_siswa.id_jurusan
										FROM tr_ikut_ujian
